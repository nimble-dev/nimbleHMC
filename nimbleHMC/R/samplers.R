...
# Running questions
# initial stepsize 1?
# deltaMax - where in STan?
# init_buffer, term_buffer, adapt_window - where in nim?
...
sampler_HMC2 <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    nwarmup        <- extractControlElement(control, 'nwarmup',        -1)
    maxTreeDepth   <- extractControlElement(control, 'maxTreeDepth',   10)
    adapt_window   <- extractControlElement(control, 'adaptWindow',    25)
    init_buffer    <- extractControlElement(control, 'initBuffer',     75)
    term_buffer    <- extractControlElement(control, 'termBuffer',     50)
    ## node list generation
    targetNodes <- model$expandNodeNames(target)
    if(length(targetNodes) <= 0) stop('HMC sampler must operate on at least one node', call. = FALSE)
    targetNodesAsScalars <- model$expandNodeNames(targetNodes, returnScalarComponents = TRUE)
    ...
    treebranchNL <- nimbleList(p_beg = double(1), p_end = double(1),
                               rho = double(1), log_sum_weight = double())
  ...
  run = function() {
    ...
    while((depth < max_depth) & (!done)) { ## CHECK 0/1 offset of these
      checkInterrupt()
      ...
      if(runif(1,0,1) > 0.5) {
        ...
        branch$p_end <- p_ff
        branch$rho <- rho_f
        branch$log_sum_weight <- log_sum_wt_subtree
        valid_subtree <- buildtree(depth, state_propose, branch, H0, 1)
        log_sum_wt_subtree <- branch$log_sum_weight
        rho_f <- branch$rho
        p_ff <- branch$p_end
        p_fb <- branch$p_begin
        copy_state(state_f, state_current)
      ...
      if(!done) {
        depth <- depth + 1
        accept <- FALSE
        log_accept_prob <- log_sum_wt_subtree - log_sum_weight
  ...
  methods = list(
    ...
    drawMomentumValues = function(state = stateNL()) {
      for(i in 1:d)   state$p[i] <- rnorm(1, 0, sqrtM[i])
    },
    inverseTransformStoreCalculate = function(qArg = double(1)) {
      values(model, targetNodes) <<- my_parameterTransform$inverseTransform(qArg)
      lp <- model$calculate(calcNodes)
    ...
    leapfrog = function(state = stateNL(), eps = double()) {
      state$p <- state$p + 0.5*eps * stateNL$gr_logProb
      state$q <- state$q +     eps * state$p / M
      update_state_calcs(state)
      state_p <- state$p + 0.5*eps * stateNL$gr_logProb
    },
    ...
    buildtree = function(depth = integer(),
      ...
      if(depth == 0) {
        ...
        if(isnan(new_H)) new_H <- Inf
        ## NEED H0
        deltaH <- new_H - H0
        if(deltaH > max_deltaH) divergent <<- TRUE
        ## Need log_sum_wt to propagate back, not locally
        branch$log_sum_wt <- log_sum_exp(branch$log_sum_wt, -deltaH)
        ## Ditto for sum_metropolis_prob
        if((-deltaH) > 0) sum_metropolis_prob <<- sum_metropolis_prob + 1
        else sum_metropolis_prob <<- sum_metropolis_prob + exp(-deltaH)
        copy_state(state_propose, state_current)
        # Need to propogate rho up
        branch$rho <- branch$rho + state_current$p
        branch$p_begin <- state_current$p
        branch$p_end <- state_current$p
        return(!divergent)
      }
      init_branch <- treebranchNL$new()
      init_branch$log_sum_weight <- -Inf
      init_branch$p_begin <- branch$p_begin
      init_branch$rho <- numeric(0, length = d)
      valid_init <- build_tree(depth-1, state_propose, init_branch, H0, sign)
      branch$p_begin <- init_branch$p_begin
      ...
      final_branch$log_sum_weight <- -Inf
      final_branch$rho <- numeric(0, length = d)
      valid_final <- build_tree(depth-1, state_propose_final, final_branch, H0, sign)
      branch$p_end <- final_branch$p_end
      if(!valid_final) return(FALSE)

      log_sum_weight_subtree <- log_sum_exp(init_branch$log_sum_weight, final_branch$log_sum_weight)
      branch$log_sum_weight <- log_sum_exp(branch$log_sum_weight, log_sum_weight_subtree)

      accept <- FALSE
      log_accept_prob <- final_branch$log_sum_weight - log_sum_weight_subtree
      if(log_accept_prob > 0) {
        accept <- TRUE
      } else {
      ...
      persist_criterion <- decide_persist(branch$p_begin, branch$p_end,
                                          init_branch$p_end,final_branch$p_begin,
                                          rho_subtree, final_branch$rho, init_branch$rho )

      ## p_sharp_begin <- branch$p_begin / M
      ## p_sharp_end   <- branch$p_end / M

      ## persist_criterion <- compute_criterion(p_sharp_begin, p_sharp_end, rho_subtree)

      ## if(persist_criterion) {
      ##   rho_subtree <- init_branch$rho + final_branch$p_begin
      ##   persist_criterion <- compute_criterion(p_sharp_begin, final_branch$p_begin / M, rho_subtree)
      ## }

      ## if(persist_criterion) {
      ##   rho_subtree <- final_branch$rho + init_branch$p_end
      ##   persist_criterion <- compute_criterion(init_branch$p_end / M, p_sharp_end, rho_subtree)
      ## }
      return(persist_criterion)
      returnType(logical())
    },
    decide_persist = function(p_b, p_e,
                              p_b2, p_e2,
                              rho, rho_1, rho_2
                              ) {
      p_b_overM <- p_b / M
    ...
    compute_criterion = function(poverM1 = double(1), poverM2 = double(1), rho = double(1)) {
      ans <- (inner_prod(poverM2, rho)) > 0 & (inner_prod(poverM1, rhow) > 0)
    },
    initializeEpsilon = function() {
      ## Close to Stan's init_stepsize
      ## CHECK ON USE OF NOM_EPSILON and always vs sometimes doing this
      initValues <- values(model, calcNodes)

      state_init <- stateNL$new()
      state_copy(state_init, state_current)
      ...
      if(isnan(newH)) newH <- Inf
      deltaH <- H0 - newH

      direction <- 2*nimStep(delta_H > log(0.8)) - 1 # 1 or -1

      done <- FALSE
      while(!done) {
        state_copy(state_current, state_init)
    ...
    adaptStepsize = function(adapt_stat = double()) {
    ...
    adapt_M = function() {
      ...
      in_adaptation_window <- (adapt_window_counter > adapt_init_bufffer) &
        (adapt_window_counter <= nwarmup - adapt_term_buffer) &
        (adapt_window_counter != nwarmup + 1) # last condition seems redundant, but following Stan closely

      if(in_adaptation_window) {
        warmupSamples[adapt_window_iter, 1:d] <<- state_sample$q
      }

      end_adapatation_window <- (adapt_window_counter == adapt_next_window) &
        (adapt_window_counter != nwarmup + 1) # Ditto comment

      if(end_adaptation_window) {
        for(i in 1:d) {
          v <- var(warmupSamples[1:adapt_window_iter, i])
          vReg <- (adapt_window_iter/(adapt_window_iter+5))*v + 0.001*(5/(adapt_window_iter+5))
          M[i] <<- 1/vReg
          sqrtM[i] <<- sqrt(M[i])
        }
        if(adapt_next_window == nwarmup - adapt_term_buffer) {
          # done, no further adaptation
          setSize(warmupSamples, 0, 0)
        } else {
          adapt_window_size <<- adapt_window_size * 2
          adapt_next_window <<- adapt_window_counter + adapt_window_size
          if(adapt_next_window != nwarmup - adapt_term_buffer) {
            next_window_boundary <- adapt_next_window + 2*adapt_window_size
            if(next_window_boundary > nwarmup - adapt_term_buffer) {
              adapt_next_window <<- nwarmup - adapt_term_buffer
              adapt_window_size <<- adapt_next_window - adapt_window_counter
            }
          }
          setSize(warmupSamples, adapt_window_size, d, copy = FALSE, fillZeros = FALSE)
        }
        adapt_window_iter <<- 1
        adapt_window_counter <<- adapt_window_counter + 1
        return(TRUE)
      }
      adapt_window_iter <<- adapt_window_iter + 1
      adapt_window_counter <<- adapt_window_counter + 1
      return(FALSE)
    },
    before_chain = function(MCMCniter = double(), MCMCnburnin = double(), MCMCchain = double()) {
      ...
      adapt_window_size <<- adapt_window # RHS is adapt_base_window in Stan
      adapt_init_buffer <<- init_buffer
      adapt_term_buffer <<- term_buffer
      adapt_next_window <<- adapt_init_buffer + adapt_window_size
      adapt_window_counter <<- 1
      adapt_window_iter <<- 1
      Hbar <<- 0
      logEpsilonBar <<- 0
      stepsizeCounter <<- 0
      setSize(warmupSamples, adapt_window_size, d, fillZeros = FALSE)
    },
    after_chain = function() {
      if(messages) {
        if(numDivergences == 1) print('  [Note] HMC sampler (nodes: ', targetNodesToPrint, ') encountered ', numDivergences, ' divergent path.')
